...
trades_total = Counter(
    'stocksx_trades_total',
    'Total number of trades',
    ['strategy', 'side', 'symbol']
)

trades_volume = Gauge(
    'stocksx_trades_volume',
    'Trading volume',
    ['strategy']
)

position_value = Gauge(
    'stocksx_position_value',
...
# 策略指標
strategy_score = Gauge(
    'stocksx_strategy_score',
    'Strategy performance score',
    ['strategy', 'grade']
)

strategy_sharpe = Gauge(
    'stocksx_strategy_sharpe',
    'Strategy Sharpe ratio',
    ['strategy']
)

strategy_drawdown = Gauge(
    'stocksx_strategy_drawdown',
...
# 風險指標
risk_exposure = Gauge(
    'stocksx_risk_exposure',
    'Total risk exposure'
)

risk_utilization = Gauge(
    'stocksx_risk_utilization',
    'Risk utilization percentage'
)

# 系統指標
system_uptime = Gauge(
    'stocksx_system_uptime',
    'System uptime in seconds'
)

websocket_status = Gauge(
    'stocksx_websocket_status',
    'WebSocket connection status (1=connected, 0=disconnected)'
)

api_latency = Histogram(
    'stocksx_api_latency_seconds',
    'API request latency',
    ['endpoint']
)
...
signals_total = Counter(
    'stocksx_signals_total',
    'Total number of signals',
    ['strategy', 'signal_type']
)

signals_active = Gauge(
    'stocksx_signals_active',
...
def main():
    ...
    logger.info(f"✅ 指標服務已啟動")
    logger.info(f"📡 指標端點：http://localhost:{port}/metrics")
    logger.info(f"📊 Grafana 配置：prometheus.yml")
    logger.info("=" * 80)
