...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/performance_monitor.log"), logging.StreamHandler()],
)
...
class PerformanceMonitor:
    ...
    def __init__(self, config_file: str = "config/monitor_config.json", alert_thresholds: dict = None):
        """
        初始化監控系統
...
"""
        ...
        for name, data in sorted(
            self.strategy_performance.items(), key=lambda x: x[1]["metrics"]["sharpe_ratio"], reverse=True
        ):
            m = data["metrics"]
            html += f"""
        <tr>
