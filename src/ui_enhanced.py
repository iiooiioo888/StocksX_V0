...
def get_searchable_items() -> Dict[str, List[Dict[str, Any]]]:
    """取得可搜索的項目（快取 5 分鐘）"""
    from src.config import CRYPTO_CATEGORIES, TRADITIONAL_CATEGORIES, STRATEGY_LABELS
    ...
    def handle_search(query: str):
        ...
        for sym in items["symbols"]:
            if (query_lower in sym["name"].lower() or 
                query_lower in sym["full_name"].lower() or
                query_lower in sym["category"].lower() or
                any(query_lower in tag for tag in sym["tags"])):
                results.append({
                    "type": "symbol",
        ...
        for strat in items["strategies"]:
            if (query_lower in strat["name"].lower() or 
                query_lower in strat["key"].lower() or
                any(query_lower in tag for tag in strat["tags"])):
                results.append({
                    "type": "strategy",
                    "icon": "📊",
                    "title": strat["name"],
...
def render_enhanced_metrics(history: List[Dict[str, Any]]):
    ...
    metric_cols[0].metric(
        "🎯 勝率",
        f"{win_rate:.1f}%",
        delta=f"{profitable_count}/{total_trades}"
    )
    metric_cols[1].metric(
        "📈 Sharpe",
        f"{sharpe:.2f}",
        delta="年化" if sharpe > 1 else "注意風險"
    )
    metric_cols[2].metric(
        "⚖️ 盈虧比",
        f"{profit_ratio:.2f}",
        delta=f"{avg_win:.1f}% / {avg_loss:.1f}%"
    )
    metric_cols[3].metric(
        "📉 最大連虧",
        f"{max_consecutive_loss}次",
        delta="風險警示" if max_consecutive_loss > 5 else "正常"
    )
    metric_cols[4].metric(
        "💰 平均獲利",
        f"{avg_win:+.1f}%",
        delta=f"{len(wins)}次獲利"
    )
    metric_cols[5].metric(
        "💸 平均虧損",
        f"{-avg_loss:.1f}%",
        delta=f"{len(losses)}次虧損",
        delta_color="inverse"
    )
    ...
    with st.expander("⚙️ 快速設定", expanded=False):
        ...
        with col1:
            default_equity = st.number_input(
                "預設初始資金",
                min_value=100.0,
                value=float(settings.get("default_equity", 10000)),
                step=500.0
            )
            default_leverage = st.number_input(
                ...
                min_value=1.0,
                max_value=125.0,
                value=float(settings.get("default_leverage", 1)),
                step=0.5
            )
        ...
        with col2:
            default_timeframe = st.selectbox(
                ...
                index=["1m", "5m", "15m", "1h", "4h", "1d"].index(
                    settings.get("default_timeframe", "1h")
                )
            )
            theme = st.selectbox(
                "主題",
