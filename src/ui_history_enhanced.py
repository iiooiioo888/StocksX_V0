    ...
    with col2:
        ...
        if total_pages > 10:
            if current_page <= 5:
                ...
                page_options = (
                    [1, f"...{current_page - 1}"] + [current_page] + [f"...{current_page + 1}"] + [total_pages]
                )
        ...
        selected_page = st.selectbox(
            "頁碼", page_options, index=min(current_page - 1, len(page_options) - 1), key=f"{key_prefix}_page_select"
        )
...
def filter_history(
    ...
    if min_return is not None:
        filtered = [item for item in filtered if item.get("metrics", {}).get("total_return_pct", 0) >= min_return]
...
def render_filters(key_prefix: str = "filter") -> dict[str, Any]:
    ...
    with st.expander("🔍 篩選條件", expanded=False):
        ...
        with col1:
            ...
            _strategy_labels = ["全部"] + [STRATEGY_LABELS.get(k, k) for k in STRATEGY_LABELS.keys()]
        ...
        with col2:
            ...
            min_return = st.number_input(
                "最小報酬率 %", min_value=-100.0, max_value=1000.0, value=None, step=1.0, key=f"{key_prefix}_min_return"
            )

        with col3:
            # 收藏篩選
            favorite_filter = st.radio("收藏", ["全部", "僅收藏", "未收藏"], index=0, key=f"{key_prefix}_favorite")
...
def sort_history(data: list[dict], sort_by: str = "created_at", ascending: bool = False) -> list[dict]:
    """
    排序歷史記錄
...
def render_sort_controls(current_sort: str, current_order: bool, key_prefix: str = "sort") -> tuple:
    ...
    with col1:
        sort_options = {"created_at": "📅 日期", "return": "💰 報酬率", "sharpe": "📊 Sharpe", "win_rate": "🎯 勝率"}
    ...
    with col2:
        order = st.radio(
            "順序", ["遞減", "遞增"], index=0 if not current_order else 1, horizontal=True, key=f"{key_prefix}_order"
        )
        ascending = order == "遞增"
...
def render_comparison_selector(data: list[dict], max_select: int = 5, key_prefix: str = "compare") -> list[dict]:
    """
    渲染對比選擇器
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        for item in data:
            ...
            rows.append(
                {
                    "ID": item.get("id"),
                    "日期": datetime.fromtimestamp(item.get("created_at", 0), tz=timezone.utc).strftime(
                        "%Y-%m-%d %H:%M"
                    ),
                    "策略": STRATEGY_LABELS.get(item.get("strategy", ""), item.get("strategy")),
                    "交易對": item.get("symbol", ""),
...
def render_statistics(data: list[dict]):
    ...
    stat_cols[0].metric("📊 總回測數", str(total_count), delta=f"{profitable_count} 筆獲利")

    stat_cols[1].metric("🎯 勝率", f"{win_rate:.1f}%", delta=f"平均 {avg_return:+.1f}%")

    stat_cols[2].metric("💰 最佳報酬", f"{best_return:+.1f}%", delta="單筆最佳")

    stat_cols[3].metric("💸 最差報酬", f"{worst_return:+.1f}%", delta="單筆最差", delta_color="inverse")

    stat_cols[4].metric("📈 平均 Sharpe", f"{avg_sharpe:.2f}", delta="風險調整後")

    stat_cols[5].metric("⭐ 收藏數量", str(sum(1 for item in data if item.get("is_favorite"))), delta="我的收藏")
    ...
    if total_count >= 5:
        ...
        fig.add_trace(
            go.Histogram(
                x=returns, nbinsx=20, marker_color=["#00cc96" if r > 0 else "#ef553b" for r in returns], opacity=0.75
            )
        )
