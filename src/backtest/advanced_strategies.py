...
def ichimoku_strategy(
    ...
    for i in range(kijun_period + 1, len(df)):
        ...
        if position == 0:
            if row["close"] > max(row["senkou_a"], row["senkou_b"]) and prev_row["close"] <= max(
                prev_row["senkou_a"], prev_row["senkou_b"]
            ):
                # 開多倉
                position = 1
                entry_price = row["close"] * (1 + slippage)
                trades.append({"timestamp": row["timestamp"], "action": "BUY", "price": entry_price, "type": "LONG"})

            elif row["close"] < min(row["senkou_a"], row["senkou_b"]) and prev_row["close"] >= min(
                prev_row["senkou_a"], prev_row["senkou_b"]
            ):
                # 開空倉
                position = -1
                entry_price = row["close"] * (1 - slippage)
                trades.append({"timestamp": row["timestamp"], "action": "SELL", "price": entry_price, "type": "SHORT"})
    ...
    return {"equity_curve": equity_curve, "trades": trades, "metrics": metrics, "raw_ohlcv": ohlcv, "error": None}
...
def vwap_reversion_strategy(
    ...
    df["vwap"] = (typical_price * df["volume"]).rolling(window=vwap_period).sum() / df["volume"].rolling(
        window=vwap_period
    ).sum()
...
ADVANCED_STRATEGIES = {
    ...
    "hull_ma": {
        ...
        "category": "趨勢",
        "params": {"hull_period": {"type": "int", "default": 9, "min": 5, "max": 50}},
    },
    "vwap_reversion": {
        "name": "VWAP 回歸",
