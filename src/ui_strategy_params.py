...
def render_strategy_params(
    strategy: str, current_params: dict[str, Any] | None = None, key_prefix: str = "strategy_params"
) -> dict[str, Any]:
    """
    渲染策略參數輸入
    ...
    for idx, (param_name, param_config) in enumerate(param_items):
        ...
        # 取得當前值或預設值
        default_value = (
            current_params.get(param_name, param_config["default"]) if current_params else param_config["default"]
        )
...
def render_all_strategy_params(
    ...
    params_dict: dict[str, dict[str, Any]] | None = None,
...
def save_params_preset(user_id: int, strategy: str, preset_name: str, params: dict[str, Any]) -> bool:
    """
    儲存參數預設
    ...
    """
    db = UserDB()
    config = {"strategy": strategy, "params": params, "created_at": __import__("time").time()}
    result = db.save_preset(user_id, preset_name, config)
    return isinstance(result, int) and result > 0


def load_params_preset(user_id: int, preset_id: int) -> dict[str, Any] | None:
    """
    載入參數預設
...
def render_preset_manager(
    user_id: int, strategy: str, current_params: dict[str, Any], key_prefix: str = "preset"
) -> dict[str, Any] | None:
    """
    渲染預設管理器
    ...
    with col1:
        ...
        if strategy_presets:
            ...
            selected_name = st.selectbox(
                "載入預設", options=list(preset_options.keys()), key=f"{key_prefix}_load_select"
            )
    ...
    with col2:
        # 儲存新預設
        new_preset_name = st.text_input("新預設名稱", placeholder=f"{strategy}_preset_1", key=f"{key_prefix}_new_name")
    ...
    with col3:
        # 刪除預設
        if strategy_presets:
            delete_name = st.selectbox("刪除", options=list(preset_options.keys()), key=f"{key_prefix}_delete_select")
...
def render_param_shortcuts(strategy: str, key_prefix: str = "shortcuts") -> dict[str, Any] | None:
    """
    渲染參數快捷方式
