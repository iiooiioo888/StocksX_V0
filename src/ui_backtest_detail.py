...
def render_backtest_detail(record: dict[str, Any]):
    ...
    if trades:
        ...
        for trade in trades:
            trade_data.append(
                {
                    "時間": datetime.fromtimestamp(trade.get("timestamp", 0) / 1000, tz=timezone.utc).strftime(
                        "%Y-%m-%d %H:%M"
                    ),
                    "方向": "🟢 多頭"
                    if trade.get("side", 0) == 1
                    else ("🔴 空頭" if trade.get("side", 0) == -1 else "⚪"),
                    "操作": trade.get("action", ""),
...
def render_backtest_comparison(records: list[dict[str, Any]]):
    ...
    st.dataframe(comparison_df, use_container_width=True, hide_index=True, height=200)
