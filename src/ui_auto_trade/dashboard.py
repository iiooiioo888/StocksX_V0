...
def render_auto_trading_dashboard(user_id: int):
    ...
    with col2:
        col2.metric(
            "總權益",
            f"${total_equity + total_pnl:,.2f}",
            delta=f"${total_equity:,.0f} 初始"
        )
    ...
    with col4:
        col4.metric(
            "勝率",
            f"{win_rate:.1f}%",
            delta=f"{winning_trades}/{total_trades} 筆"
        )
    ...
    if not strategies:
        ...
        for idx, strategy in enumerate(strategies):
            ...
            with st.expander(
                ...
                with action_col3:
                    if st.button("🗑️ 刪除", key=f"del_strat_{strategy['id']}", use_container_width=True, type="secondary"):
