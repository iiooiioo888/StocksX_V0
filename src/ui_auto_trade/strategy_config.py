    ...
    with st.form("strategy_config_form"):
        ...
        with ex_col1:
            exchange_id = st.selectbox(
                "交易所", options=["binance", "okx", "bybit", "gateio"], index=0, help="選擇要連接的交易所"
            )
        ...
        for i in range(num_subscriptions):
            ...
            with sub_col1:
                symbol = st.text_input(
                    "交易對", placeholder="BTC/USDT:USDT", value="BTC/USDT:USDT" if i == 0 else "", key=f"symbol_{i}"
                )
            ...
            with sub_col3:
                timeframe = st.selectbox(
                    "時間框架", options=["1m", "5m", "15m", "1h", "4h", "1d"], index=3, key=f"timeframe_{i}"
                )
        ...
        with risk_col1:
            risk_per_trade = (
                st.slider(
                    "每筆風險 (%)", min_value=0.5, max_value=5.0, value=2.0, step=0.5, help="每筆交易願意承擔的風險比例"
                )
                / 100
            )
        ...
        with st.expander("⚙️ 進階風險設定"):
            ...
            with adv_col2:
                ...
                close_on_stop = st.toggle("任務結束時平倉", value=True, help="自動交易停止時自動平掉所有持倉")
    ...
    if strategy == "sma_cross":
        ...
        with col1:
            params["fast_period"] = st.number_input(
                "快線", min_value=1, max_value=50, value=12, key=f"p_macd_f_{index}"
            )
        with col2:
            params["slow_period"] = st.number_input(
                "慢線", min_value=1, max_value=100, value=26, key=f"p_macd_s_{index}"
            )
        with col3:
            params["signal_period"] = st.number_input(
                "信號線", min_value=1, max_value=50, value=9, key=f"p_macd_sig_{index}"
            )
        ...
        with col2:
            params["oversold"] = st.number_input(
                "超賣線", min_value=10, max_value=40, value=30, key=f"p_rsi_os_{index}"
            )
        with col3:
            params["overbought"] = st.number_input(
                "超買線", min_value=60, max_value=90, value=70, key=f"p_rsi_ob_{index}"
            )
        ...
        with col2:
            params["std_dev"] = st.number_input(
                "標準差", min_value=1.0, max_value=3.0, value=2.0, step=0.1, key=f"p_bb_s_{index}"
            )
        ...
        with col2:
            params["slow_period"] = st.number_input(
                "慢線", min_value=1, max_value=200, value=21, key=f"p_ema_s_{index}"
            )
        ...
        with col2:
            params["multiplier"] = st.number_input(
                "倍數", min_value=1.0, max_value=5.0, value=3.0, step=0.1, key=f"p_st_m_{index}"
            )
        ...
        with col2:
            params["oversold"] = st.number_input(
                "超賣", min_value=-90, max_value=-50, value=-80, key=f"p_wr_os_{index}"
            )
        with col3:
            params["overbought"] = st.number_input(
                "超買", min_value=-50, max_value=-10, value=-20, key=f"p_wr_ob_{index}"
            )
        ...
        with col1:
            params["af_start"] = st.number_input(
                "初始 AF", min_value=0.01, max_value=0.1, value=0.02, step=0.01, key=f"p_sar_s_{index}"
            )
        with col2:
            params["af_step"] = st.number_input(
                "AF 步長", min_value=0.01, max_value=0.1, value=0.02, step=0.01, key=f"p_sar_i_{index}"
            )
        with col3:
            params["af_max"] = st.number_input(
                "最大 AF", min_value=0.1, max_value=0.5, value=0.2, step=0.05, key=f"p_sar_m_{index}"
            )
        ...
        with col2:
            params["k1"] = st.number_input(
                "K1 (多頭)", min_value=0.1, max_value=2.0, value=0.5, step=0.1, key=f"p_dt_k1_{index}"
            )
        with col3:
            params["k2"] = st.number_input(
                "K2 (空頭)", min_value=0.1, max_value=2.0, value=0.5, step=0.1, key=f"p_dt_k2_{index}"
            )
        ...
        with col2:
            params["threshold"] = st.number_input(
                "閾值", min_value=0.5, max_value=5.0, value=2.0, step=0.1, key=f"p_vwap_t_{index}"
            )

    elif strategy == "ichimoku":
        with col1:
            params["tenkan_period"] = st.number_input(
                "轉換線", min_value=5, max_value=50, value=9, key=f"p_ichi_t_{index}"
            )
        with col2:
            params["kijun_period"] = st.number_input(
                "基準線", min_value=10, max_value=100, value=26, key=f"p_ichi_k_{index}"
            )
        with col3:
            params["senkou_b_period"] = st.number_input(
                "先行線 B", min_value=20, max_value=200, value=52, key=f"p_ichi_s_{index}"
            )
