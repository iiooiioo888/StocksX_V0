...
from .nlp_strategies.sentiment_analyzer import NewsMonitor, SentimentAnalyzer
...
class AdvancedStrategiesManager:
    ...
    def __init__(self):
        """初始化策略管理器"""
        # LSTM 策略
        self.lstm_predictor = LSTMPredictor(lookback=60, forecast_horizon=5, lstm_units=50, dropout_rate=0.2)
        ...
        # 配对交易
        self.pairs_trading = PairsTrading(lookback_window=60, entry_zscore=2.0, exit_zscore=0.5, stoploss_zscore=3.0)

        # 策略状态
        self.strategy_states = {}

    def add_news(self, title: str, content: str, source: str, symbols: list[str] | None = None):
        """添加新闻"""
        self.news_monitor.add_news(
            title=title, content=content, source=source, published_at=datetime.now(), symbols=symbols
        )
    ...
    def get_lstm_signal(self, df: pd.DataFrame) -> dict:
        ...
        try:
            if self.lstm_predictor.model is None:
                return {"strategy": "lstm", "status": "model_not_trained", "message": "请先训练 LSTM 模型"}
            ...
            return {"strategy": "lstm", "status": "error", "error": str(e)}
    ...
    def get_sentiment_signal(self, hours: int = 24) -> dict:
        ...
        try:
            ...
            return {"strategy": "sentiment", "status": "error", "error": str(e)}

    def get_pairs_trading_signal(self, price1: pd.Series, price2: pd.Series) -> dict:
        """
        获取配对交易信号
        ...
        try:
            ...
            return {"strategy": "pairs_trading", "status": "error", "error": str(e)}

    def get_ensemble_signal(
        self,
        df: pd.DataFrame,
        price1: pd.Series | None = None,
        price2: pd.Series | None = None,
        ...
        """
        if weights is None:
            weights = {"lstm": 0.4, "sentiment": 0.3, "pairs_trading": 0.3}
        ...
        if not signals:
            return {"strategy": "ensemble", "status": "no_signals", "message": "没有可用的策略信号"}
    ...
    def train_lstm(
        self, df: pd.DataFrame, epochs: int = 50, batch_size: int = 32, model_path: str | None = None
    ) -> dict:
        """
        训练 LSTM 模型
        ...
        """
        try:
            history = self.lstm_predictor.train(df, epochs=epochs, batch_size=batch_size, model_path=model_path)
    ...
    def find_cointegrated_pairs(self, price_data: dict[str, pd.Series], p_value_threshold: float = 0.05) -> list[dict]:
        """
        寻找协整的股票对
    ...
    def get_all_signals(self) -> dict:
        """获取所有策略的当前信号"""
        return {"timestamp": datetime.now().isoformat(), "signals": self.strategy_states.copy()}
