...
class AntiMartingale(RiskManagementStrategy):
    ...
    def calculate_position_size(
        self, signal: int, capital: float, price: float, volatility: float, last_pnl: float = 0
    ) -> float:
        """
        計算倉位大小
...
class FixedRatio(RiskManagementStrategy):
    ...
    def __init__(self, risk_per_trade: float = 0.02, delta: float = 0.01):
        """
        初始化固定比率法
        ...
        Args:
            ...
            delta: 預期利潤率（默认 1%）
        """
        super().__init__("固定比率法", {"risk_per_trade": risk_per_trade, "delta": delta})
...
class CVaRPositionSizing(RiskManagementStrategy):
    ...
    def __init__(self, confidence_level: float = 0.95, max_cvar: float = 0.05, lookback: int = 252):
        """
        初始化 CVaR 倉位控制
        ...
        """
        super().__init__(
            "CVaR/ES 倉位控制", {"confidence_level": confidence_level, "max_cvar": max_cvar, "lookback": lookback}
        )
    ...
    def calculate_position_size(
        self, signal: int, capital: float, price: float, volatility: float, returns: pd.Series = None
    ) -> float:
        """
        計算倉位大小
...
class OptimalStopLoss(RiskManagementStrategy):
    ...
    def __init__(self, base_stop_loss: float = 0.02, volatility_adjustment: bool = True, time_decay: float = 0.001):
        """
        初始化最優停損
    ...
    def calculate_optimal_stop(self, current_price: float, current_volatility: float, holding_period: int = 0) -> float:
        """
        計算最優止損位
    ...
    def should_stop(self, current_price: float, current_volatility: float, holding_period: int = 0) -> bool:
        """
        判斷是否應該止損
        ...
        Returns:
            是否止損
        """
        stop_price = self.calculate_optimal_stop(current_price, current_volatility, holding_period)
...
class TailRiskHedge(RiskManagementStrategy):
    ...
    def __init__(self, hedge_ratio: float = 0.05, vix_threshold: float = 20, max_hedge_cost: float = 0.02):
        """
        初始化尾部風險對沖
        ...
        super().__init__(
            "尾部風險對沖",
            {"hedge_ratio": hedge_ratio, "vix_threshold": vix_threshold, "max_hedge_cost": max_hedge_cost},
        )
    ...
    def calculate_hedge_ratio(self, current_vix: float = None) -> float:
        ...
        if current_vix is not None:
            ...
            if current_vix > vix_threshold:
                hedge_ratio = min(hedge_ratio * (current_vix / vix_threshold), max_hedge_cost)

        return hedge_ratio

    def should_hedge(self, portfolio_value: float, current_vix: float = None) -> tuple[bool, float]:
        """
        判斷是否應該對沖
...
class DynamicDeltaHedge(RiskManagementStrategy):
    ...
    def __init__(self, rebalance_threshold: float = 0.1, max_delta: float = 0.05):
        ...
        """
        super().__init__("動態對沖", {"rebalance_threshold": rebalance_threshold, "max_delta": max_delta})
...
class VolatilityTargeting(RiskManagementStrategy):
    ...
    def __init__(
        self, target_volatility: float = 0.15, max_position: float = 1.0, min_position: float = 0.1, lookback: int = 60
    ):
        """
        初始化波動率目標
...
if __name__ == "__main__":
    ...
    for name, strategy in strategies:
        ...
        try:
            # 測試倉位計算
            position = strategy.calculate_position_size(signal=1, capital=100000, price=100, volatility=0.02)
