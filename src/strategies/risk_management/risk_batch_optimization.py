...
class CVaRPositionOptimized(RiskManagementStrategy):
    ...
    def __init__(
        ...
        use_dynamic_cvar: bool = True,
    ):
        super().__init__(
            "CVaR Position Optimized",
            {
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        # 計算 CVaR
        cvar = returns.rolling(window=lookback).apply(lambda x: self.calculate_cvar(x, confidence))
...
class OptimalStopOptimized(RiskManagementStrategy):
    ...
    def __init__(
        self, atr_period: int = 14, atr_multiplier: float = 2.0, use_trailing_stop: bool = True, trail_pct: float = 0.05
    ):
        super().__init__(
            "Optimal Stop Optimized",
            {
...
def main():
    ...
    cvar_best, cvar_results = optimize_strategy(
        data,
        CVaRPositionOptimized,
        {"confidence_level": [0.95, 0.975, 0.99], "lookback": [126, 252], "max_position_pct": [0.2, 0.3]},
        "CVaR",
    )
    ...
    stop_best, stop_results = optimize_strategy(
        data, OptimalStopOptimized, {"atr_period": [10, 14, 18], "atr_multiplier": [1.5, 2.0, 2.5]}, "Optimal Stop"
    )
    ...
    delta_best, delta_results = optimize_strategy(
        data,
        DynamicDeltaHedgeOptimized,
        {"rebalance_threshold": [0.05, 0.1, 0.15], "hedge_ratio": [0.3, 0.5]},
        "Delta Hedge",
    )
