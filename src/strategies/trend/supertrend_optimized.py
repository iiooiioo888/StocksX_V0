...
class SupertrendOptimized(TrendFollowingStrategy):
    ...
    def calculate_adaptive_multiplier(self, data: pd.DataFrame) -> pd.Series:
        ...
        # 標準化波動率
        vol_normalized = (volatility - volatility.rolling(window=vol_lookback).mean()) / volatility.rolling(
            window=vol_lookback
        ).std()
...
def main():
    ...
    # 創建回測引擎
    print("\n[2/4] 初始化回測引擎...")
    backtester = SupertrendBacktester(initial_capital=100000.0, commission_rate=0.001, slippage=0.001)

    # 參數網格搜索
    print("\n[3/4] 執行參數網格搜索...")
    param_grid = {"period": [8, 10, 12], "multiplier": [2.5, 3.0, 3.5]}
