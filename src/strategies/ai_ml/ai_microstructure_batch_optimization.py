...
class VPINOptimized(BaseStrategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        # VPIN 模擬
        vpin = (buy_volume - sell_volume).rolling(window=bucket_size).sum() / volume.rolling(window=bucket_size).sum()
...
class Level2DepthOptimized(BaseStrategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        # 標準化
        depth_zscore = (depth_imbalance - depth_imbalance.rolling(window=50).mean()) / depth_imbalance.rolling(
            window=50
        ).std()
...
def main():
    ...
    lstm_best, lstm_results = optimize_strategy(
        data, LSTMPredictorOptimized, {"lookback": [10, 20, 30], "threshold": [0.01, 0.02]}, "LSTM Predictor"
    )

    # 優化 Transformer
    print("\n[3/7] 優化 Transformer...")
    transformer_best, transformer_results = optimize_strategy(
        data, TransformerOptimized, {"attention_window": [5, 10, 15], "signal_threshold": [0.01, 0.015]}, "Transformer"
    )

    # 優化 DQN
    print("\n[4/7] 優化 DQN...")
    dqn_best, dqn_results = optimize_strategy(
        data, DQNAgentOptimized, {"learning_rate": [0.05, 0.1, 0.2], "epsilon": [0.05, 0.1]}, "DQN Agent"
    )

    # 優化 VPIN
    print("\n[5/7] 優化 VPIN...")
    vpin_best, vpin_results = optimize_strategy(
        data, VPINOptimized, {"bucket_size": [30, 50, 100], "threshold": [1.5, 2.0]}, "VPIN"
    )

    # 優化 Level 2
    print("\n[6/7] 優化 Level 2 Depth...")
    level2_best, level2_results = optimize_strategy(
        data, Level2DepthOptimized, {"depth_levels": [3, 5, 10], "imbalance_threshold": [0.2, 0.3]}, "Level 2 Depth"
    )
