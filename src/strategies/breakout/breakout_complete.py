...
class NR7NR4(BreakoutStrategy):
    ...
    def __init__(self, nr7_lookback: int = 7, nr4_lookback: int = 4):
        super().__init__("NR7/NR4", {"nr7_lookback": nr7_lookback, "nr4_lookback": nr4_lookback})
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        # NR7: 過去 7 日最小範圍
        nr7 = range_val.rolling(window=lookback_7).apply(lambda x: x.iloc[-1] == x.min())

        # NR4: 過去 4 日最小範圍
        nr4 = range_val.rolling(window=lookback_4).apply(lambda x: x.iloc[-1] == x.min())
...
class FlagPennant(BreakoutStrategy):
    ...
    def __init__(self, flag_length: int = 10, breakout_lookback: int = 5):
        super().__init__("旗形/三角旗形", {"flag_length": flag_length, "breakout_lookback": breakout_lookback})
...
class HorizontalChannel(BreakoutStrategy):
    ...
    def __init__(self, channel_length: int = 20, threshold: float = 0.03):
        super().__init__("水平通道突破", {"channel_length": channel_length, "threshold": threshold})
...
class WMPattern(BreakoutStrategy):
    ...
    def __init__(self, lookback: int = 50, tolerance: float = 0.02):
        super().__init__("W 底/M 頂", {"lookback": lookback, "tolerance": tolerance})
...
class TTOOpeningRange(BreakoutStrategy):
    ...
    def __init__(self, ibr_multiplier: float = 0.5):
        super().__init__("TTO Opening Range", {"ibr_multiplier": ibr_multiplier})
