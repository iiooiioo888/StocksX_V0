...
class BollingerSqueezeOptimized(BreakoutStrategy):
    ...
    def get_squeeze_strength(self, data: pd.DataFrame) -> pd.Series:
        ...
        # 計算歷史百分位數
        squeeze_strength = bandwidth.rolling(window=lookback).apply(lambda x: (x.iloc[-1] < x).mean() * 100)
...
def main():
    ...
    # 創建回測引擎
    print("\n[2/4] 初始化回測引擎...")
    backtester = BollingerSqueezeBacktester(initial_capital=100000.0, commission_rate=0.001, slippage=0.001)

    # 參數網格搜索
    print("\n[3/4] 執行參數網格搜索...")
    param_grid = {"period": [18, 20, 22], "std_dev": [1.8, 2.0, 2.2], "squeeze_threshold": [0.03, 0.05, 0.07, 0.09]}
