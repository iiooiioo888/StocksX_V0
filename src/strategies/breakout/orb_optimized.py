...
class OpeningRangeBreakoutOptimized(BreakoutStrategy):
    ...
    def __init__(self, 
        ...
        """
        super().__init__('Opening Range Breakout Optimized', {
    ...
    def check_exit_conditions(self, data: pd.DataFrame, position: int, 
    ...
    def get_range_strength(self, data: pd.DataFrame) -> pd.Series:
        ...
        # 計算歷史百分位數
        range_percentile = range_size.rolling(window=250).apply(
...
class ORBBacktester:
    ...
    def backtest(self, data: pd.DataFrame, strategy: OpeningRangeBreakoutOptimized) -> Dict:
        ...
        for i in range(1, len(data)):
            ...
            # 檢查止盈/止損
            if position != 0:
                position, entry_price = strategy.check_exit_conditions(
                    data.iloc[:i+1], position, entry_price
                )
                if position == 0:
