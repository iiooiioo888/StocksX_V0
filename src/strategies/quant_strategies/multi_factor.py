...
class MultiFactorModel:
    ...
    def __init__(
        self,
        factors: List[str] = ['market', 'size', 'value', 'momentum', 'quality', 'low_volatility']
    ):
        """
        初始化多因子模型
    ...
    def calculate_low_volatility_factor(
        self,
        returns: pd.Series,
        lookback: int = 252
    ) -> float:
...
class MultiFactorStrategy:
    ...
    def __init__(
        self,
        model: Optional[MultiFactorModel] = None,
    ...
    def rank_stocks(
        ...
        if 'market_cap' in df.columns:
            ...
            df['size_score'] = df['market_cap'].apply(
                lambda x: self.model.calculate_size_factor(x, median_cap)
            )
        ...
        if 'pe_ratio' in df.columns and 'pb_ratio' in df.columns:
            ...
            df['value_score'] = df.apply(
                lambda row: self.model.calculate_value_factor(
                    row['pe_ratio'], row['pb_ratio'], median_pe, median_pb
                ),
                axis=1
            )
        ...
        if 'roe' in df.columns and 'debt_to_equity' in df.columns:
            ...
            df['quality_score'] = df.apply(
                lambda row: self.model.calculate_quality_factor(
                    row['roe'], row.get('roa', 0), row['debt_to_equity'],
                    median_roe, median_de
                ),
                axis=1
            )
        ...
        for factor, scores in factor_scores.items():
            weight = {'size': 0.15, 'value': 0.25, 'momentum': 0.25, 'quality': 0.20, 'low_volatility': 0.15}.get(factor, 0)
...
if __name__ == "__main__":
    ...
    for stock in stocks[:3]:
        ...
        value_score = model.calculate_value_factor(
            row['pe_ratio'], row['pb_ratio'],
            stock_data['pe_ratio'].median(), stock_data['pb_ratio'].median()
        )
