...
class PairsTrading:
    ...
    def __init__(
        self, lookback: int = 60, entry_zscore: float = 2.0, exit_zscore: float = 0.5, stoploss_zscore: float = 3.0
    ):
        """
        初始化配对交易策略
        ...
        self.mean_spread: float | None = None
        self.std_spread: float | None = None

    def check_cointegration(
        self, price_a: pd.Series, price_b: pd.Series, confidence: float = 0.05
    ) -> tuple[bool, float]:
        """
        检验两个价格序列是否协整
    ...
    def calculate_spread(self, price_a: pd.Series, price_b: pd.Series, hedge_ratio: float | None = None) -> pd.Series:
        """计算价差"""
    ...
    def generate_signal(self, price_a: pd.Series, price_b: pd.Series) -> dict:
        ...
        if len(price_a) < self.lookback or len(price_b) < self.lookback:
            return {"strategy": "pairs_trading", "signal": 0, "action": "WAIT", "reason": "数据不足"}

        # 计算对冲比率
        self.hedge_ratio = self.calculate_hedge_ratio(price_a.iloc[-self.lookback :], price_b.iloc[-self.lookback :])
    ...
    def backtest(self, price_a: pd.Series, price_b: pd.Series, initial_capital: float = 100000) -> pd.DataFrame:
        """
        回测配对交易策略
        ...
        for i in range(self.lookback, len(price_a)):
            signal = self.generate_signal(price_a.iloc[:i], price_b.iloc[:i])
