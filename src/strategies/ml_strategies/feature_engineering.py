...
class FeatureEngineer:
    ...
    def create_technical_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for window in [20]:
            ...
            df[f'bb_position'] = (df['close'] - df[f'bb_lower_{window}']) / (df[f'bb_upper_{window}'] - df[f'bb_lower_{window}'] + 1e-10)
    ...
    def fit_scaler(self, df: pd.DataFrame, columns: Optional[List[str]] = None):
        """拟合标准化器"""
