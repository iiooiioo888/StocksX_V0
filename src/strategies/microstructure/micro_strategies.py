...
class OrderFlowAnalysis(BaseStrategy):
    ...
    def __init__(self, lookback: int = 20, imbalance_threshold: float = 0.3):
        ...
        """
        super().__init__(
            "訂單流分析", {"lookback": lookback, "imbalance_threshold": imbalance_threshold}, category="microstructure"
        )
...
class CumulativeDelta(BaseStrategy):
    ...
    def __init__(self, lookback: int = 50):
        ...
        Args:
            lookback: 回看週期
        """
        super().__init__("Delta 累積", {"lookback": lookback}, category="microstructure")
...
class POCValueArea(BaseStrategy):
    ...
    def __init__(self, lookback: int = 100, va_percentage: float = 0.7):
        ...
        """
        super().__init__(
            "POC / Value Area", {"lookback": lookback, "va_percentage": va_percentage}, category="microstructure"
        )
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        vwap = (close * volume).rolling(window=lookback).sum() / volume.rolling(window=lookback).sum()
...
class VPIN(BaseStrategy):
    ...
    def __init__(self, bucket_size: int = 50, lookback: int = 100):
        ...
        Args:
            ...
            lookback: 回看週期
        """
        super().__init__("VPIN", {"bucket_size": bucket_size, "lookback": lookback}, category="microstructure")
...
class AmihudIlliquidity(BaseStrategy):
    ...
    def __init__(self, lookback: int = 60):
        ...
        Args:
            lookback: 回看週期
        """
        super().__init__("Amihud 非流動性", {"lookback": lookback}, category="microstructure")
...
class IcebergDetection(BaseStrategy):
    ...
    def __init__(self, lookback: int = 20, volume_threshold: float = 3.0):
        ...
        """
        super().__init__(
            "冰山訂單偵測", {"lookback": lookback, "volume_threshold": volume_threshold}, category="microstructure"
        )
...
class MicroPrice(BaseStrategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        # 微價格 = 加權平均價
        micro_price = (data["close"] * data["volume"]).rolling(window=lookback).sum() / data["volume"].rolling(
            window=lookback
        ).sum()
