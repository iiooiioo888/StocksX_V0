...
class CointegrationPairOptimized(BaseStrategy):
    ...
    def __init__(self, lookback: int = 60, threshold: float = 2.0,
                 use_dynamic_threshold: bool = True, vol_adjustment: bool = True):
        super().__init__('Cointegration Pair Optimized', {
            'lookback': lookback,
            'threshold': threshold,
...
class KalmanFilterOptimized(BaseStrategy):
    ...
    def __init__(self, process_variance: float = 1e-5, measurement_variance: float = 0.1,
                 signal_threshold: float = 0.02, use_adaptive_alpha: bool = True):
        super().__init__('Kalman Filter Optimized', {
...
class MarketMakingOptimized(BaseStrategy):
    ...
    def __init__(self, spread_pct: float = 0.01, order_size_pct: float = 0.05,
                 inventory_limit: float = 0.2, skew_factor: float = 0.5):
        super().__init__('Market Making Optimized', {
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        # 計算中間價（使用 VWAP 近似）
        vwap = (data['close'] * data['volume']).rolling(window=10).sum() / \
...
def main():
    ...
    coint_best, coint_results = optimize_strategy(
        data, CointegrationPairOptimized,
        {
            'lookback': [40, 60, 80],
    ...
    kalman_best, kalman_results = optimize_strategy(
        data, KalmanFilterOptimized,
        {
            'process_variance': [1e-5, 1e-4],
            'measurement_variance': [0.05, 0.1, 0.2],
    ...
    mm_best, mm_results = optimize_strategy(
        data, MarketMakingOptimized,
        {
            'spread_pct': [0.005, 0.01, 0.02],
            'order_size_pct': [0.03, 0.05],
            'skew_factor': [0.3, 0.5]
        },
        'Market Making'
    )
