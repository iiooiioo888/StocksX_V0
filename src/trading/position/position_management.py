...
class PositionStrategy(ABC):
    ...
    @abstractmethod
    def calculate_position_size(self, capital: float, price: float, **kwargs) -> float:
        """
        计算仓位大小
...
class KellyCriterion(PositionStrategy):
    ...
    def __init__(
        self, win_rate: float, profit_loss_ratio: float, kelly_fraction: float = 0.5, max_position: float = 0.25
    ):
        """
        初始化
...
class PyramidingPosition(PositionStrategy):
    ...
    def calculate_position_size(
        self, capital: float, price: float, current_position: Optional[PositionInfo] = None, **kwargs
    ) -> float:
        """
        计算仓位大小
...
class MartingalePosition(PositionStrategy):
    ...
    def calculate_position_size(self, capital: float, price: float, consecutive_losses: int = 0, **kwargs) -> float:
        """
        计算仓位大小
...
class RiskParityPosition(PositionStrategy):
    ...
    def calculate_weights(self, volatilities: dict[str, float]) -> dict[str, float]:
        """
        计算风险平价权重
    ...
    def calculate_position_size(
        self, capital: float, price: float, volatility: float, total_volatility: float, **kwargs
    ) -> float:
        """
        计算单个资产的仓位大小
...
class PositionManager:
    ...
    def calculate_new_position(self, symbol: str, price: float, **kwargs) -> float:
        """
        计算新仓位
        ...
        return self.strategy.calculate_position_size(
            capital=self.total_capital, price=price, current_position=current_pos, **kwargs
        )
...
if __name__ == "__main__":
    ...
    # 模拟持仓
    position = PositionInfo(symbol="BTC/USDT", side="buy", entry_price=50000, current_price=52500, amount=1.0)
    ...
    manager.add_position(
        PositionInfo(symbol="BTC/USDT", side="buy", entry_price=50000, current_price=52000, amount=0.5)
    )
