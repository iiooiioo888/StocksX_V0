...
class BlackLittermanOptimizer:
    ...
    def __init__(
        ...
        if market_cap_weights:
            self.market_weights = np.array([
                market_cap_weights.get(a, 1.0 / self.n_assets)
    ...
    def add_views(
        ...
        for i, view in enumerate(views):
            ...
            # 置信度越高，Ω 越小（观点越确定）
            p_row = P[i, :]
            omega_diag[i] = (1 - confidence) * np.dot(
                p_row, np.dot(self.tau * self.cov_matrix, p_row)
            )
    ...
    def optimize_no_views(self) -> Dict[str, Any]:
        ...
        port_vol = np.sqrt(
            np.dot(self.market_weights.T, np.dot(self.cov_matrix, self.market_weights))
        )
        sharpe = (port_return - self.risk_free_rate) / port_vol if port_vol > 0 else 0
