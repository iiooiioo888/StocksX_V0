...
class StatisticalArbitrage:
    ...
    def test_cointegration(self, asset_a: str, asset_b: str) -> dict[str, Any]:
        """
    ...
    def compute_spread(self, asset_a: str, asset_b: str, hedge_ratio: Optional[float] = None) -> pd.Series:
        """
        计算价差序列
    ...
    def compute_zscore(self, spread: pd.Series, window: Optional[int] = None) -> pd.Series:
        """
        计算 Z-Score
    ...
    def scan_pairs(self, asset_pairs: list[tuple[str, str]]) -> list[dict[str, Any]]:
        """
        批量扫描配对
    ...
    def generate_signal(
        self, asset_a: str, asset_b: str, current_prices: Optional[dict[str, float]] = None
    ) -> PairSignal:
        """
        生成交易信号
    ...
    def backtest_pair(self, asset_a: str, asset_b: str, initial_capital: float = 100000.0) -> dict[str, Any]:
        """
        回测配对交易策略
        ...
        hedge_ratio = self._cointegration_cache.get((asset_a, asset_b), {}).get("hedge_ratio", 1.0)
...
class FundingRateArbitrage:
    ...
    def fetch_funding_rate(self, exchange, symbol: str) -> Optional[dict[str, Any]]:
        """
        获取资金费率
    ...
    def scan_opportunities(
        self, symbols: list[str], exchange_instances: Optional[dict[str, Any]] = None
        ...
        for exchange_id in self.exchanges:
            ...
            for symbol in symbols:
                ...
                # 预估利润（假设持仓 30 天）
                hold_days = 30
                est_profit = abs(rate) * self.settlements_per_day * hold_days * self.max_position_usd
                ...
                logger.info(
                    f"发现资金费率机会：{exchange_id} {symbol} | 费率={rate:.4%} ({direction}) | 年化={annualized:.1f}%"
                )
    ...
    def calculate_optimal_hedge(
        self, spot_price: float, perpetual_price: float, funding_rate: float, position_usd: float, hold_days: int = 30
    ) -> dict[str, Any]:
        """
        计算最优对冲方案
    ...
    def estimate_risk(
        self, spot_price: float, perpetual_price: float, position_usd: float, volatility: float = 0.02
    ) -> dict[str, Any]:
        """
        估算套利风险
