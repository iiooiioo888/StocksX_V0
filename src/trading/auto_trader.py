...
from .executor import OrderResult, TradeExecutor, create_executor_from_config
from .risk_manager import RiskConfig, RiskManager, create_risk_manager_from_config
...
class AutoTrader:
            ...
            if current_position == 0:
                ...
                if signal == 0:
                    ...
                    self._close_position(
                        symbol, current_position, strategy, "信號消失", config
                    )
                elif signal == -current_position:
                    # 信號反轉，平倉並反向開倉
                    self._reverse_position(
                        symbol, current_position, signal, strategy, config
                    )
                else:
                    # 信號維持，檢查停損/停利
                    self._check_stop_loss_take_profit(
                        symbol, current_position, position_info, config
                    )
    ...
    def _try_open_position(
        ...
        position_size = self.risk_manager.calculate_position_size(
            equity, current_price, stop_loss_price
        )
        ...
        if result.success:
            ...
            logger.info(
                f"✅ 開倉成功：{side.upper()} {position_size} {symbol} "
                f"@ {current_price} (策略：{strategy})"
            )
        else:
    ...
    def _close_position(
        ...
        if result.success:
            ...
            logger.info(
                f"✅ 平倉成功：{symbol} {reason} | "
                f"損益：{pnl_pct:+.2f}% (${equity * pnl_pct / 100:+,.2f})"
            )
        else:
