...
class RateLimiter:
    ...
    def allow_request(
        ...
        if self._redis:
            ...
            return self._allow_request_redis(
                key, tokens, capacity, refill_rate, wait, max_wait
            )
        else:
            # 記憶體模式
            return self._allow_request_memory(
                key, tokens, capacity, refill_rate, wait, max_wait
            )
    ...
    def _allow_request_redis(
        ...
        if wait and wait_time > 0 and wait_time <= max_wait:
            ...
            # 重試
            return self._allow_request_redis(
                key, tokens, capacity, refill_rate, False, max_wait
            )
...
def get_api_limiter(api_name: str) -> Tuple[RateLimiter, Dict[str, Any]]:
    ...
    config = API_LIMIT_CONFIG.get(api_name, {
        "capacity": 10,
        "refill_rate": 1.0
    })
