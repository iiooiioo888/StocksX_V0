...
def check_redis(redis_url: str | None = None) -> HealthStatus:
    """檢查 Redis 健康狀態"""
    start = time.time()
...
def check_ccxt(exchange_id: str = "binance") -> HealthStatus:
    ...
    try:
        ...
        return HealthStatus(
            service=f"ccxt:{exchange_id}",
            status="unhealthy",
            message=f"Exchange error: {e!s}",
            latency_ms=(time.time() - start) * 1000,
        )


def check_celery_broker(broker_url: str | None = None) -> HealthStatus:
    ...
    try:
        ...
        return HealthStatus(
            service="celery_broker",
            status="unhealthy",
            message=f"Celery broker error: {e!s}",
            latency_ms=(time.time() - start) * 1000,
        )
...
def check_memory() -> HealthStatus:
    ...
    try:
        ...
        return HealthStatus(
            service="memory",
            status="degraded",
            message=f"Memory check error: {e!s}",
        )
...
def render_health_page():
    ...
    st.set_page_config(page_title="StocksX - 健康檢查", page_icon="🏥", layout="wide")
    ...
    st.markdown(f"### 整體狀態：{status_colors.get(health['status'], '⚪')} {health['status'].upper()}")
