...
class AutomatedBacktestPipeline:
    ...
    def load_strategies(self, strategy_dir: str = None) -> Dict[str, BaseStrategy]:
        ...
        if strategy_dir is None:
            strategy_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 
                                       'src', 'strategies')
        ...
        for root, dirs, files in os.walk(strategy_dir):
            for file in files:
                if file.endswith('_optimized.py') and not file.startswith('__'):
                    try:
                        module_name = file[:-3]
                        module = __import__(f'{os.path.basename(root)}.{module_name}', 
                                          fromlist=[''])
    ...
    def backtest_single(self, args) -> Dict:
        ...
        try:
            ...
            sortino = np.sqrt(252) * returns.mean() / returns[returns < 0].std() if returns[returns < 0].std() > 0 else 0
    ...
    def run_backtest(self, 
                     ...
                     default_params: Dict = None) -> Dict[str, Dict]:
        """
        執行批量回測
...
- **回報最高**: {max(sorted_results, key=lambda x: x[1]['total_return'])[0]} ({max(sorted_results, key=lambda x: x[1]['total_return'])[1]['total_return']*100:.2f}%)
- **回撤最小**: {min(sorted_results, key=lambda x: x[1]['max_drawdown'])[0]} ({min(sorted_results, key=lambda x: x[1]['max_drawdown'])[1]['max_drawdown']*100:.2f}%)
...
def main():
    ...
    # 初始化 Pipeline
    pipeline = AutomatedBacktestPipeline(
