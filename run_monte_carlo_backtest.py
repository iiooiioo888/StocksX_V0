...
def run_large_scale_backtest():
    ...
    print("="*80)
    print(f"模拟次数：10,000 次/策略")
    print(f"测试策略：33 个")
    print(f"总模拟次数：330,000 次")
    print()
    ...
    for category, strategy_list in strategies.items():
        ...
        for strategy_name in strategy_list:
            ...
            try:
                # 运行蒙特卡洛模拟
                stats, details_df = monte_carlo_backtest(
                    strategy_name,
                    n_simulations=10000,
                    days=1095
                )
...
def generate_large_scale_report(all_stats: List[Dict], all_details: Dict):
    ...
    category_stability = stats_df.groupby('category').agg({
    ...
    distribution = stats_df[[
        ...
        'return_5pct', 'median_return', 'return_95pct',
...
{stats_df.nlargest(10, 'profit_probability')[['strategy_name', 'category', 'profit_probability', 'mean_return']].to_markdown(index=False)}

### 按夏普比率中位数（风险调整）

{stats_df.nlargest(10, 'median_sharpe')[['strategy_name', 'category', 'median_sharpe', 'mean_return']].to_markdown(index=False)}

### 按最坏回撤（极端风险）

{stats_df.nsmallest(10, 'worst_drawdown')[['strategy_name', 'category', 'worst_drawdown', 'mean_return']].to_markdown(index=False)}

---

## 📈 各类别对比

{stats_df.groupby('category').agg({
    'profit_probability': 'mean',
    'mean_return': 'mean',
    'median_sharpe': 'mean',
    'worst_drawdown': 'mean',
...
{stats_df.nlargest(15, 'median_return')[['strategy_name', 'return_5pct', 'median_return', 'return_95pct']].to_markdown(index=False)}
