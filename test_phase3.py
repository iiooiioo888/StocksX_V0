...
def test_feature_engineering():
    ...
    try:
        ...
        fe = FeatureEngineer()
        print(f"✅ 特征工程：初始化成功")
        print(f"   - 可用指标：趋势、动量、波动率、成交量、布林带等")
        return True
    except Exception as e:
        print(f"❌ 特征工程：{e}")
        return False
...
def test_sentiment():
    ...
    try:
        ...
        print(f"✅ NLP 情绪分析：初始化成功")
        print(f"   - 模型：FinBERT (prosusai/finbert)")
        print(f"   - 功能：新闻监控、情绪聚合、信号生成")
        return True
    except Exception as e:
        print(f"❌ NLP 情绪分析：{e}")
        return False
...
def test_advanced_manager():
    ...
    try:
        ...
        print(f"✅ 高级策略管理器：初始化成功")
        print(f"   - 集成策略：LSTM + NLP + 配对交易 + 多因子")
        return True
    except Exception as e:
        print(f"❌ 高级策略管理器：{e}")
        return False
