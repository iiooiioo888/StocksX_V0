...
# 創建 FastAPI 應用
app = FastAPI(
    title="StocksX API", description="130 策略量化交易系統 API", version="8.0.0", docs_url="/docs", redoc_url="/redoc"
)
...
async def signal_pusher():
    ...
    while True:
        ...
        if signals:
            await manager.broadcast({"type": "signals", "data": signals, "count": len(signals)})
...
async def websocket_signals(websocket: WebSocket):
    ...
    try:
        ...
        await manager.broadcast({"type": "system", "message": f"用戶 {websocket.client.host} 離線"})
...
async def health_check():
    """健康檢查端點"""
    return {"status": "healthy", "timestamp": datetime.now().isoformat(), "version": "8.0.0", "strategies": 130}
