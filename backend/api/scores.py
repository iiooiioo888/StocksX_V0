...
def get_score_ranking(top_n: int = Query(50, ge=1, le=200), db: Session = Depends(get_db)):
    """獲取評分排名"""
    scores = db.query(Score).join(Strategy).order_by(Score.score.desc()).limit(top_n).all()
...
def create_or_update_score(strategy_name: str, score_data: ScoreResponse, db: Session = Depends(get_db)):
    """創建或更新策略評分"""
    strategy = db.query(Strategy).filter(Strategy.name == strategy_name).first()
...
def get_grade_distribution(db: Session = Depends(get_db)):
    ...
    distribution = (
        db.query(Score.grade, func.count(Score.id).label("count")).group_by(Score.grade).order_by(Score.grade).all()
    )
