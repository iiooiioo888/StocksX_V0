...
st.set_page_config(page_title="组合优化 - StocksX", page_icon="📊", layout="wide")
...
# 选择优化方法
optimization_method = st.sidebar.selectbox("优化方法", ["🎯 最大夏普比率", "🛡️ 最小波动率", "⚖️ 风险平价", "📈 有效前沿"])

# 资产选择
st.sidebar.subheader("📊 资产选择")
default_assets = ["BTC", "ETH", "SOL", "BNB", "USDT"]
selected_assets = st.sidebar.multiselect("选择资产", default_assets, default=default_assets)
...
if run_optimization or True:  # 默认显示
    with st.spinner("正在优化组合..."):
        ...
        stats_data = {
            ...
            "夏普比率": [
                (returns[col].mean() * 252 - risk_free_rate) / (returns[col].std() * np.sqrt(252))
        ...
        if optimization_method == "🎯 最大夏普比率":
            ...
            portfolio_return = sum(
                weights[a] * stats_df[stats_df["资产"] == a]["年化收益"].values[0] for a in selected_assets
            )
            portfolio_volatility = np.sqrt(
                sum(
                    weights[a] ** 2 * (stats_df[stats_df["资产"] == a]["年化波动"].values[0]) ** 2
