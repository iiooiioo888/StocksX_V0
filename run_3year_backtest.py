...
# 回测结果存储
backtest_results = {"classic": {}, "ai": {}, "arbitrage": {}, "optimization": {}}
...
def run_backtest():
    ...
    print("=" * 80)
    print("回测周期：2023-03-20 至 2026-03-20（3 年）")
    print("初始资金：$100,000")
    print()
...
def generate_report(results: dict):
    ...
    for category, strategies in results.items():
        if not strategies:
            continue
        best = max(
            [(name, s["sharpe_ratio"]) for name, s in strategies.items() if "sharpe_ratio" in s], key=lambda x: x[1]
        )
